# Projects

## Client Portal (Cortez Digital)
| Field | Value |
|-------|-------|
| **Tech Stack** | Python, Flask, SQLite, HTML, CSS, JS, Chart.js |
| **Lines of Code** | ~1,350 |
| **Features** | User auth, admin panel, file uploads, email notifications, charts, pagination, search/filter |
| **Status** | ✅ Working locally |
| **GitHub** | https://github.com/vinicio-cortez/client-portal |

## SEO Audit Script
| Field | Value |
|-------|-------|
| **Tech Stack** | Python |
| **Description** | Processes Screaming Frog CSV → JSON findings |
| **Status** | ✅ Complete |
| **GitHub** | https://github.com/vinicio-cortez/seo-audit-pipeline |

## Notion Portfolio
| Field | Value |
|-------|-------|
| **Description** | Professional portfolio page |
| **Status** | ✅ Live |
| **Link** | (add your Notion link here) |

## Stuart Restaurant SEO Audit
| Field | Value |
|-------|-------|
| **Description** | Technical SEO audit with branded report |
| **Tools** | Screaming Frog, Ubersuggest, PageSpeed Insights |
| **Status** | ✅ Complete |